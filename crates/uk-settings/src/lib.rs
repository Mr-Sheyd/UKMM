...
pub struct Settings {
    ...
    pub lang: LocLang,
    pub suppress_bnp: bool,
    pub suppress_install: bool,
    pub suppress_merge: bool,
}
...
impl Settings {
    ...
    pub fn load() -> RwLock<Settings> {
        RwLock::new(match Settings::read(Self::path()) {
            Ok(settings) => {
                log::debug!("{:#?}", settings);
